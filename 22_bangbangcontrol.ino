...
// Servo adjustment
#define _DUTY_MAX 2183  // Servo angle: D degree
#define _DUTY_NEU 1700  // Servo angle: 0 degree
#define _DUTY_MIN 737  // Servo angle: E degree
...
void loop() {
  ...
  if(event_dist) {
    ...
    if(dist_ema < dist_target) { // Replace 1 with a proper test expression using dist_target and dist_ema.
      //duty_target = ; // Complete this line using _BANGBANG_RANGE.
...
// Replace the below line with the equation obtained from nonlinear regression analysis
  ...
  return 1903 + -14.5*x + 0.0438*pow(x,2) + -6.03*pow(10,-5)*pow(x,3) + 3.11*pow(10,-8)*pow(x,4);
}
